...
<head>
  ...
  <title>E-Biblio | Change & Remove Admins</title>
...
<body id="top">
  ...
  <div class="page-banner">
    ...
    <!-- Page Banner Title -->
    <h1 class="page-banner__title">Change / Remove Admin Users</h1>
  </div>
  ...
  <section class="admin-change-admins">
    <div class="admin-change-admins__container container">
      ...
      <h3 class="admin-change-admins__description">Use this grid to update user details and delete users as required.</h3>

      <!-- Search bar component -->
      <form class="admin-change-admins__search-bar search-bar" action="admin_change_admins.php" method="POST">
      ...
      <div class="admin-change-admins__data-table data-table">
        ...
        <hr class="data-table__hr admin-change-admins__hr">
        ...
        if ($searchTerm != '') {
          $query = mysqli_query($connection, "SELECT * FROM users WHERE (admin = 'admin') AND (username LIKE '%$searchTerm%' OR password LIKE '%$searchTerm%') ORDER BY username");
        }
        ...
        while ($row = mysqli_fetch_array($query)) {
          extract($row);
          echo "<form class='data-table__form' action='admin_change_admins.php' method='POST'>";
